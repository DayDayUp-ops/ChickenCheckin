...
- 主题区域新增全局透明度调节，可同步控制首页、生活记录和数据统计中的外层卡片、打卡信息块、小日历、输入框、任务与记录以及状态信息块；透明度设置随账号同步。
- 自定义背景会保留最近一次上传的图片；切换到固定主题色后，可再次点击“自定义背景”恢复，重复点击可更换图片。
- 今日打卡的“中午”改为“下午”，晚上打卡时间调整为17:00～20:00。
- 加深正文和辅助文字颜色，提高透明卡片及自定义背景下的文字对比度和清晰度。
- 云同步改为以云端记录为唯一权威来源：登录和刷新时自动采用云端最新数据，不再弹出本机与云端版本选择；新账号从空白云端记录开始，退出账号时清除本机记录缓存。
- 修复启动阶段自动拉取、宜忌保存和延迟上传相互触发造成的重复刷新；云端数据改为直接更新当前页面，不再通过整页刷新生效。
